...
/**
 * An implementation of CustomFieldSerilizer for serializing {@link OWLAnnotationImpl}
 * objects.
 */
public class OWLAnnotationImpl_CustomFieldSerializer extends CustomFieldSerializer<OWLAnnotationImpl> {
     ...
     */
    @Override
    public OWLAnnotationImpl instantiateInstance(SerializationStreamReader streamReader) throws SerializationException {
        return instantiate(streamReader);
    }

    public static OWLAnnotationImpl instantiate(SerializationStreamReader streamReader) throws SerializationException {
        OWLAnnotationProperty property = (OWLAnnotationProperty) streamReader.readObject();
        OWLAnnotationValue value = (OWLAnnotationValue) streamReader.readObject();
        int annotationsSize = streamReader.readInt();
        Set<OWLAnnotation> annotations = new HashSet<OWLAnnotation>(annotationsSize);
        for(int i = 0; i < annotationsSize; i++) {
            OWLAnnotation anno = (OWLAnnotation) streamReader.readObject();
    ...
    public static void serialize(SerializationStreamWriter streamWriter, OWLAnnotationImpl instance) throws SerializationException {
        streamWriter.writeObject(instance.getProperty());
        streamWriter.writeObject(instance.getValue());
        streamWriter.writeInt(instance.getAnnotations().size());
        for(OWLAnnotation anno : instance.getAnnotations()) {
            streamWriter.writeObject(anno);
        }
    }
    ...
    public static void deserialize(SerializationStreamReader streamReader, OWLAnnotationImpl instance) throws SerializationException {
